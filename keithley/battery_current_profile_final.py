#!/usr/bin/env python3
"""
Keithley 2281S - Bataryaya Manuel Akım Profili FINAL
V1'in çalışan yapısını kullanarak, tam CSV profilini uygular
"""
...
LOG_FILE = 'battery_current_profile_final.csv'
...
def setup_manual_mode(inst):
    """Cihazı manuel kontrol moduna al - V1 ile aynı"""
...
def apply_current(inst, current_value):
    """Belirli bir akım değeri uygula - V1 ile aynı"""
    if current_value >= 0:
...
def get_measurement(inst):
    """Ölçüm al - V1 ile aynı (READ fallback ile)"""
    try:
        # Direct measurement
...
def main():
    ...
    print("\nSeçenekler:")
    print("1. İlk 50 satırı test et (hızlı)")
    print("2. Tüm CSV'yi uygula (tam test)")
    choice = input("Seçiminiz (1/2): ").strip()
    
    if choice == "1":
        df = df.head(50)
        print(f"İlk 50 satır seçildi")
    else:
    ...
    with open(LOG_FILE, 'w', newline='') as f:
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                print(f"{idx+1:4d}: {mode:9s} | Hedef {target_current:7.4f} A, Ölçülen {current:7.4f} A, {voltage:.3f} V")

                # 1 saniye bekle
                time.sleep(0.7)
                
                # Her 100 satırda duraklatma seçeneği
                if (idx + 1) % 100 == 0:
                    print(f"\n{idx+1} satır tamamlandı. Devam etmek için Enter, durdurmak için Ctrl+C...")
                    try:
                        input()
