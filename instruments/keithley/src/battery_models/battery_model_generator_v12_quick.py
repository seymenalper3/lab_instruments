...
CONFIG = {
    'discharge': {
        'voltage': 3.4,      # V-empty (discharge end voltage) - CHANGED for quick test
        'current_end': 0.5,  # End current for discharge (A) - CHANGED for faster
    },
    'charge': {
        'v_full': 4.00,      # Full charge voltage (V) - CHANGED for quick test
        'i_limit': 1.00,     # Max charging current (A)
        'esr_interval': 5,   # ESR measurement interval (s) - CHANGED for more data points
    },
    'model': {
        'v_min': 3.4,        # Model voltage range minimum - CHANGED to match discharge
        'v_max': 4.0,       # Model voltage range maximum - CHANGED to match charge
        'slot': 4,           # Internal memory slot (1-9)
    },
    'timeouts': {
        'discharge_max_hours': 0.5,  # CHANGED: 30 minutes max
        'charge_max_hours': 1,        # CHANGED: 1 hour max
        'poll_interval': 10,          # CHANGED: Check every 10 seconds
    }
}
...
class BatteryModelGenerator:
    ...
    def connect(self):
        ...
        try:
            # Try PyVISA-py backend first (pure Python)
            try:
    ...
    def get_measurement_status(self):
        ...
        try:
            ...
            # Try to get voltage/current from buffer or measurement
            voltage = 0.0
            current = 0.0
            
            try:
                # First try to get last measurement from buffer
                data = self.query(':BATT:DATA:DATA:SEL? 1,1,"VOLT,CURR"')
                if data:
                ...
                # If buffer fails, try direct measurement (may not work in all states)
                try:
    ...
    def wait_measurement_complete(self, max_hours, poll_interval=30):
        ...
        # Initial wait for stabilization
        time.sleep(2)
        
        while True:
            status = self.get_measurement_status()
            if not status:
                self.logger.warning("Status read failed, retrying...")
                time.sleep(5)
                continue
                
            # Log progress
            elapsed = time.time() - start_time
            if status['voltage'] > 0 or status['current'] > 0:
                self.logger.info(
                ...
                self.logger.info(
                    f"Progress: {elapsed/60:.1f} min | "
                    f"Measuring: {status['measuring']} | "
                    f"Status: 0x{status['status_code']:04X}"
                )
    ...
    def discharge_battery(self):
        ...
        self.logger.info("Waiting for discharge to start...")
        time.sleep(5)
    ...
    def charge_and_characterize(self):
        ...
        self.write(':TRACe:FEED:CONT ALW')  # Continuous buffer mode
        ...
        # Wait a bit for measurement to stabilize
        self.logger.info("Waiting for charge to start...")
        time.sleep(5)
    ...
    def export_model_to_csv(self, slot):
        ...
        time.sleep(1)  # Allow time for model recall
        ...
        with open(csv_file, 'w', newline='') as f:
            ...
            # Read model data (always 101 points for complete model)
            rows_written = 0
            for i in range(101):
                try:
        ...
        self.logger.info(f"Model exported to: {csv_file} ({rows_written} rows)")
        return csv_file if rows_written > 0 else None
    ...
    def export_measurement_data(self):
        ...
        try:
            # Get buffer size
            points_str = self.query(':TRACe:POINts:ACTual?')
            ...
            self.logger.info(f"Buffer contains {points} data points")
            ...
            with open(csv_file, 'w', newline='') as f:
                ...
                for start in range(1, points + 1, chunk_size):
                    ...
                    try:
                        ...
                        self.logger.warning(f"Failed to read chunk {start}-{end}: {e}")
                        continue
                                
            self.logger.info(f"Measurement data exported to: {csv_file} ({total_rows} rows)")
            return csv_file if total_rows > 0 else None
