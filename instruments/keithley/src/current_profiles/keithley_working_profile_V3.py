        ...
        with open(self.output_file, 'w', newline='') as f:
            ...
            try:
                for idx, row in df.iterrows():
                    ...
                    # Voltajı ayarla - daha büyük fark için
                    if set_current > 0:  # Deşarj
                        # Bataryadan akım çekmek için voltajı düşük ayarla
                        # Daha yüksek akımlar için daha düşük voltaj
                        voltage_offset = min(0.5 + abs(set_current) * 0.2, 1.0)
                        self.inst.write(f':VOLT {VOLTAGE_NOMINAL - voltage_offset:.3f}')
                    else:  # Şarj
